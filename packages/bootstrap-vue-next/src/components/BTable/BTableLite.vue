    ...
    <BThead :variant="headVariant" :class="theadClass">
      ...
      <BTr :variant="headRowVariant" :class="theadTrClass">
        ...
        >
          <slot
            :name="
              $slots[`head(${field.key as string})`] ? `head(${field.key as string})` : 'head()'
            "
            :label="field.label"
            :column="field.key"
            :field="field"
      ...
      >
        ...
        <template v-for="(item, itemIndex) in items" :key="itemIndex">
          <BTr
            ...
            :variant="item._rowVariant as any"
          ...
          >
            <BTd
              v-for="field in computedFields"
              :key="field.key"
              :variant="(item._cellVariants as any)?.[field.key] ? null : field.variant"
            ...
            >
              ...
              <slot
                :name="
                  $slots[`cell(${field.key as string})`] ? `cell(${field.key as string})` : 'cell()'
                "
                :value="get(item, field.key as string)"
                :index="itemIndex"
                :item="item"
                :field="field"
                :items="items"
              ...
              >
                {{ formatItem(item, field.key as string, field.formatter) }}
              </slot>
            </BTd>
          </BTr>
        ...
        >
          <div class="d-inline-flex flex-nowrap align-items-center gap-1">
            <div>
              <slot
                :name="
                  $slots[`foot(${field.key as string})`] ? `foot(${field.key as string})` : 'foot()'
                "
                :label="field.label"
                :column="field.key"
                :field="field"
